...
from tools.expt_utils import get_unique_results_path

NETWORK_NAME = "flmms_net"
IMAGE_NAME = "weskwong/flmms:v1.0"
MNT_PATH = "$(pwd)"
RESULTS_PATH = get_unique_results_path(global_config.results_path,
                                       global_config.expt_name)
...
def run_docker(id):
    ...
    sh_cmd = f"CUDA_VISIBLE_DEVICES={global_config.cuda_device[id]} RESULTS_PATH={RESULTS_PATH} {torchrun_cmd} > {log_name} 2>&1"
...
def launch():
    ...
    for i in range(1, global_config.num_client + 1):
        run_docker(i)
    monitor_server_log(global_config.monitor_server_log)
